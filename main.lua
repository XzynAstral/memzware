...
--* Application Details *--
Name = "aimware" --* Application Name
Ownerid = "tuHrfaR8uw" --* OwnerID
...
local Rayfield = loadstring(readfile('Novoline/Assets/UiLib.lua'))()
...
local Window = Rayfield:CreateWindow({
	...
	ConfigurationSaving = {
		Enabled = true,
		FolderName = "Novoline", -- Create a custom folder for your hub/game
...
local Notification = loadstring(readfile('Novoline/Assets/Notifs.lua'))()
...
--arraylist defining
local array = loadstring(readfile('Novoline/Assets/ArrayList.lua'))()
shared["CometConfigs"] = {
    Enabled = true
}
--arraylist defined
...
do
    ...
    local Enabled = false
    local Messages = {"Pow!","Thump!","Wham!","Hit!","Smack!","Bang!","Pop!","Boom!", "NovolinePrivate!", "Kabam!", "Skuuuura!", "Ablam!", "Pha pha!", "inf", "SkidzuraOnBottom!"}
    local old
    ...
    local TexturePack = Visuals:CreateToggle({
        ...
        Callback = function(val)
            ...
            if Enabled then
                ...
                loadstring(readfile('Novoline/Assets/Textures.lua'))()
            else
    ...
    local CustomSound = Combat:CreateToggle({
        ...
        Callback = function(val)
            ...
            if Enabled then
                ...
                if game.PlaceId == 8560631822 then
                    lplr.leaderstats.Bed:GetPropertyChangedSignal("Value"):Connect(function()
                        if lplr.leaderstats.Bed.Value ~= "✅" then
                            ...
                            sound.SoundId = getasset("Novoline/Sound/mc/bedbroken.mp3") -- path to where ever the sound is in ur workspace
                            sound:Play()
                            wait(7)
                            sound:remove()
                        end
                    ...
                    spawn(function()
                        Client:WaitFor("BedwarsBedBreak"):andThen(function(p13)
                            p13:Connect(function(p14)
                                local sound = Instance.new("Sound")
                                sound.Parent = workspace
                                sound.SoundId = getasset("Novoline/Sound/mc/bedbreak.mp3")-- path to where ever the sound is in ur workspace
                                sound:Play()
                                wait(4)
                                sound:remove()
                            end)
                ...
                newsounds.PICKUP_ITEM_DROP = getasset("Novoline/Sound/mc/pickup.mp3")
                ...
                newsounds.BEDWARS_PURCHASE_ITEM = getasset("Novoline/Sound/mc/buyitem.mp3")
                newsounds.STATIC_HIT = "rbxassetid://6607204501"
                newsounds.STONE_BREAK = "rbxassetid://6496157434"
                newsounds.WOOL_BREAK = getasset("Novoline/Sound/mc/woolbreak.mp3")
                newsounds.WOOD_BREAK = getasset("Novoline/Sound/mc/breakwood.mp3")
                newsounds.GLASS_BREAK = getasset("Novoline/Sound/mc/glassbreak.mp3")
                newsounds.TNT_HISS_1 = getasset("Novoline/Sound/mc/tnthiss.mp3")
                newsounds.TNT_EXPLODE_1 = getasset("Novoline/Sound/mc/tntexplode.mp3")
                gamesound = newsounds
            else
